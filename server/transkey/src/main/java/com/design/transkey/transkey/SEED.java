...
// reorder for little endian	  
	...
	//int SeedContext_SetKey(void *pObject, unsigned char *pKey/*16byte*/);
	static void SeedEncryptEcb(byte pOutput[], byte pInput[], int pdwRoundKey[])
	{
		SeedEncrypt(pInput, pdwRoundKey, pOutput);
	}

	static void SeedDecryptEcb(byte pOutput[], byte pInput[], int pdwRoundKey[])
	{
		SeedDecrypt(pInput, pdwRoundKey, pOutput);
	}

	public static void SeedContext_SetKey(
			int pdwRoundKey[], 			// [out] round keys for encryption or decryption
			byte pKey[])
	{
		SeedRoundKey(pdwRoundKey, pKey);
	}

	public static void SeedContext_EncryptCbc(
			int pdwRoundKey[], 			// [in] round keys for encryption or decryption
			byte pIv[],
			byte pData[],
			int pLen,
			byte outData[])					// [out] data to be encrypted
	{
		int i, tBlockSize, tRemainLen;
		
		tBlockSize = (int)(pLen/16);
		tRemainLen = (int)(pLen%16);

		byte tResultOut[] = new byte[16];
		byte tResultIn[] = new byte[16];

		byte tInput1[] = new byte[16];
		byte tXorer[] = new byte[16];
		
		System.arraycopy(pIv, 0, tXorer, 0, 16);
		for(i=0; i < tBlockSize; i++)
		{
			System.arraycopy(pData, i * 16, tInput1, 0, 16);

			SEEDXor(tResultIn, tInput1, tXorer);
			SeedEncryptEcb(tResultOut, tResultIn, pdwRoundKey);	

			System.arraycopy(tResultOut, i * 16, tXorer, 0, 16);
		}
		
		//do not encrypt remain bytes.
		//remain bytes will be treated by API user.
		if( tRemainLen != 0 )
			System.arraycopy(tResultOut, i * 16, pData, 0, 16);
		
		System.arraycopy(tResultOut, 0, outData, 0, 16);
	}

	public static void SeedContext_DecryptCbc(
			int pdwRoundKey[], 			// [in] round keys for encryption or decryption
			byte pIv[],
			byte pData[],
			int pLen,
			byte outData[])					// [out] data to be decrypted
	{
		int i, tBlockSize, tRemainLen;
		...
		{
			System.arraycopy(pData, i * 16, tInput1, 0, 16);
			SeedDecryptEcb(tResultOut, tInput1, pdwRoundKey);
			SEEDXor(tResultOut, tResultOut, tXorer);
			System.arraycopy(tResultOut, i * 16, tXorer, 0, 16);
		}
...
// User secret key
		byte pbUserKey[]  = {(byte)0x0f, (byte)0x00, (byte)0x00, (byte)0x03, (byte)0x00, (byte)0x01, (byte)0x00, (byte)0x00,
		                     (byte)0x0f, (byte)0x0a, (byte)0x00, (byte)0x00, (byte)0x02, (byte)0x00, (byte)0x00, (byte)0x00};
		byte pbUserIV[]  = {(byte)0x0a, (byte)0x0a, (byte)0x00, (byte)0x0f, (byte)0x00, (byte)0x01, (byte)0x00, (byte)0x00,
                (byte)0x00, (byte)0x0a, (byte)0x0a, (byte)0x00, (byte)0x03, (byte)0x02, (byte)0x00, (byte)0x00};
...
// Decryption
	  ...
	  //
	  	...
	  	String strIn1 = "1234567890123456";
	  	String strIn2 = "qwertyuiopasdfgh";
	  	byte bIn1[] = strIn1.getBytes();
	  	byte bIn2[] = strIn2.getBytes();
	  	byte bResult[] = new byte[16];

	  	SEEDXor(bResult, bIn1, bIn2);

		byte pbSeedExPlain[]  = {(byte)0x00, (byte)0x01, (byte)0x02, (byte)0x03, (byte)0x04, (byte)0x05, (byte)0x06, (byte)0x07,
                (byte)0x08, (byte)0x09, (byte)0x0a, (byte)0x0b, (byte)0x0c, (byte)0x0d, (byte)0x0e, (byte)0x0f};
	  	
		SeedContext_SetKey(pdwSEEDExRoundKey, pbUserKey);
		SeedContext_EncryptCbc(pdwSEEDExRoundKey, pbUserIV, pbSeedExPlain, 16, bResult);
		for (int i=0; i<16; i++)	System.out.print(Integer.toHexString(0xff&bResult[i])+" ");
		System.out.print("\n");

		SeedContext_DecryptCbc(pdwSEEDExRoundKey, pbUserIV, bResult, 16, pbPlain);	  	
		for (int i=0; i<16; i++)	System.out.print(Integer.toHexString(0xff&pbPlain[i])+" ");
		System.out.print("\n");
	}
	*/	
